const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4710';
...
const adminCookie = await login('admin@identity-access.local', 'admin123');
const managerCookie = await login('manager@identity-access.local', 'manager123');
const analystCookie = await login('analyst@identity-access.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/identities', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/identities', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/identities/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/identities/approve`, {
  method: 'POST',
  headers: {
